...
import { useCategories, useFirebase } from 'src/hooks';
...
const CreateProductComponent = () => {
	const [breadcrumbs] = useState([
		...
		{ name: 'Thêm mới', path: '/quan-ly/san-pham/create', active: true },
	]);
		...
		<>
			<div className='flex flex-col'>
				...
				<div className='-mt-4 mb-8'>
					<h3 className='text-2xl'>Thêm mới sản phẩm</h3>
				</div>
				<div className='flex justify-end gap-4 w-full mb-8'>
					...
					<button className='min-w-[100px] px-4 py-[7px] bg-[#2074be] hover:bg-sky-600 text-white rounded-md flex justify-center items-center' disabled={loading} onClick={onCreate}>
						{loading ? <Loading /> : 'Lưu'}
					</button>
				</div>
				<div className='w-full mb-8'>
					<ProductForm ref={formRef} form={form} product={undefined} categories={categories}></ProductForm>
				</div>
			</div>
		</>
	);
